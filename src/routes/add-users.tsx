...
import { Search, MoreVertical, HelpCircle, Plus, CheckSquare } from "lucide-react"
...
function AddUsersPage() {
  ...
  useEffect(() => {
    const hasSeenWalkthrough = localStorage.getItem(`add-users-walkthrough-${callsign}`)
  ...
  if (!userTypeLoading && userType !== "admin") {
    return (
      <div className="max-w-4xl mx-auto space-y-6 text-center py-12">
        ...
        <p className="text-xl text-muted-foreground">{t("addUsers.forbidden")}</p>
      </div>
    )
  }

  const filteredCodes =
    inviteCodes?.filter((invite) => invite.invitecode.toLowerCase().includes(filterText.toLowerCase())) || []
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-center gap-3">
        ...
        >
          ...
          <span className="hidden md:inline">
            {createInviteCodeMutation.isLoading ? t("addUsers.creating") : t("addUsers.createNewInvite")}
          </span>
          <span className="md:hidden">{createInviteCodeMutation.isLoading ? "..." : t("addUsers.create")}</span>
        </Button>
        <Button
          variant="outline"
          onClick={() => {
      ...
      {bulkMode && selectedCodes.length > 0 && (
        <div className="flex items-center gap-2 p-4 bg-card border border-border rounded-xl">
          <span className="text-sm font-medium">{t("addUsers.selected", { count: selectedCodes.length })}</span>
        ...
        {filteredCodes.map((invite) => (
          <div
            ...
            className={cn(
              ...
              bulkMode && selectedCodes.includes(invite.invitecode) && "bg-accent border-primary",
            ...
            <div className="flex items-center gap-4 flex-1">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-mono font-bold text-lg">{invite.invitecode}</span>
                  <span
                    className={cn(
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  {invite.owner_cs === callsign
                    ? t("addUsers.createdByYou", {
                        date: invite.created ? format(new Date(invite.created), "MMM d, yyyy") : "",
                      })
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="pt-4 space-y-3 text-sm leading-relaxed text-left">
              <p className="font-semibold text-foreground">{t("addUsers.createModalWarning")}</p>
              <p>{t("addUsers.createModalText")}</p>
              <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                <li>{t("addUsers.createModalReason1")}</li>
                <li>{t("addUsers.createModalReason2")}</li>
              </ul>
              <p className="text-xs text-muted-foreground">{t("addUsers.createModalTip")}</p>
            </DialogDescription>
          </DialogHeader>
          <div className="flex gap-3 pt-4 flex-col sm:flex-row">
            <Button variant="outline" onClick={() => setCreateModalOpen(false)} className="flex-1 h-11">
              {t("addUsers.cancel")}
            </Button>
            <Button onClick={handleCreateInvite} className="flex-1 h-11 bg-primary hover:bg-primary/90">
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">{t("addUsers.walkthrough.step1Description")}</p>
            </div>
            <div className="space-y-2">
              <h4 className="font-semibold text-sm">{t("addUsers.walkthrough.step2Title")}</h4>
              <p className="text-sm text-muted-foreground">{t("addUsers.walkthrough.step2Description")}</p>
            </div>
            <div className="space-y-2">
              <h4 className="font-semibold text-sm">{t("addUsers.walkthrough.step3Title")}</h4>
              <p className="text-sm text-muted-foreground">{t("addUsers.walkthrough.step3Description")}</p>
            </div>
            <div className="space-y-2">
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="font-mono font-semibold text-foreground">{selectedCode}</span>
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-3 pt-4">
            ...
            >
              {deleteInviteCodeMutation.isLoading ? t("addUsers.deleting") : t("addUsers.deleteCode")}
            </Button>
            <Button variant="ghost" onClick={() => setManageDialogOpen(false)} className="w-full">
              {t("addUsers.cancel")}
            </Button>
          </div>
        </DialogContent>
